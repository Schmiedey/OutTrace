...
export function ShareScan({ scan, snapshot }: { scan: ScanRow; snapshot: ScanGraphSnapshot }) {
  ...
  return <>
    ...
    <dialog ref={dialog} aria-label="Preview scan share card" className="m-auto max-w-3xl rounded-lg border border-line bg-canvas p-5 text-ink backdrop:bg-black/40" onCancel={() => setPreview(null)}>
      ...
      {preview ? <img src={preview.url} width={1200} height={630} alt={`OutTrace scan of ${scan.domain}`} className="mb-4 w-full rounded border border-line" /> : null}
      <div className="flex gap-2"><Button disabled={!preview} onClick={() => { if (preview) { downloadBlob(`outtrace-${scan.domain}-scan.png`, preview.blob); noteUsage("share-card-exported"); close(); } }}>Download PNG · 1200 × 630</Button><Button variant="ghost" onClick={close}>Cancel</Button></div>
    </dialog>
  </>;
}
