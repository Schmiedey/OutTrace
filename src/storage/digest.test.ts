...
describe("native digest", () => {
  ...
  it("delivers one native summary per interval", async () => {
    ...
    expect(create).toHaveBeenCalledWith(expect.stringContaining("linkscope-digest-"), expect.objectContaining({ message: "1 site changed." }));
  });
  ...
  it("respects disabled notifications and Pro recurring entitlements", async () => {
